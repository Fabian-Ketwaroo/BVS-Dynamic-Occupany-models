...
# Rscript demonstrating BVS via PG works
...

{
  ...
  classCovariates <- c(rep(TRUE, ncov-1), FALSE) # first 4 continouous and last one categorical with 3 levels
  classCovariates
  ...
  d_bar <- ncov # prior beliefs about the number of important covariates
...
win.data <- list(S= M*T, D=S, nsamples = length(y), nsites = M, numVars = numVars, b = b_psi, B = B_psi, zindex = zindex, ncov = ncov+1, indexes_covariates= indexes_covariates )
str(win.data)
...
# Inital value for z
zints = c(apply(Y, c(1,3), max))
# Initial value for PG parameter (Omega)
up = matrix(1,N,1)
Omega_p = pgdraw(rep(1, each = N),0.5*up)
up = matrix(1,S,1)
Omega_phi = pgdraw(rep(1, each = D),0.5*up)
up = matrix(1,S,1)
Omega_eta = pgdraw(rep(1, each = D),0.5*up)
u = matrix(1,M,1)
...
set.seed(1)
{
  ...
  # Design matrix for psi
  X_psi = matrix(rnorm(M*(ncov) ), nrow = M, ncol = ncov)  # all continuous covariates 
  originalX_psi <- X_psi # backup the original  matrix of covariates
  X_psi = as.data.frame(X_psi)
  X_psi <- model.matrix(~ ., X_psi) # creates a design (or model) matrix
  # Here the design matrix has a column of ones for the intercept
  
  # here the 3rd and 5th beta are non-significant 
  true_betas_psi <- c(0.75, 0.3, 0, 0.5, 0.6, 0) # includes intercept
  true_gamma_psi <- c(1,1,0,1,1,0) # inclusion parameter
  true_gammabeta_psi <- true_gamma_psi*true_betas_psi
  
  psi <-  ilogit(X_psi %*% true_gammabeta_psi) # initial occupancy probability 
  mean(psi)
  
  # Design matrix for survival (phi) 
  X_phi = matrix(rnorm(S*(ncov) ), nrow = S, ncol = ncov)  # all continuous covariates
  originalX_phi <- X_phi # backup the original  matrix of covariates
  X_phi = as.data.frame(X_phi)
  X_phi <- model.matrix(~ ., X_phi) # creates a design (or model) matrix
  head(X_phi)
  # Here the design matrix has a column of ones for the intercept
  
  # Survival -here the 3rd and 5th beta are non-significant
  true_betas_phi <- c(1, 0.3, 0.11, 0.5, 0.6, -0.2) # includes intercept
  true_gamma_phi <- c(1,1,0,1,1,0) # inclusion parameter
  true_gammabeta_phi <- true_gamma_phi*true_betas_phi
  
  phi <-  matrix(ilogit(X_phi %*% true_gammabeta_phi),M,T-1) # occupancy probability 
  mean(apply(phi, 2, mean))
  
  
  # Design matrix for colonization (eta) 
  X_eta = X_phi
  true_betas_eta <- c(-0.5, 0.3, 0.3, 0.5, 0.6, -0.2) # includes intercept
  true_gamma_eta <- c(1,1,0,1,1,0) # inclusion parameter
  true_gammabeta_eta <- true_gamma_eta*true_betas_eta
  
  eta <-  matrix(ilogit(X_eta %*% true_gammabeta_eta),M,T-1) # occupancy probability 
  mean(apply(eta, 2, mean))
  
  
  # Design matrix for p
  X_p = matrix(rnorm(N*(ncov) ), nrow = N, ncol = ncov)  # all continuous covariates
  originalX_p <- X_p # backup the original  matrix of covariates
  X_p = as.data.frame(X_p)
  X_p <- model.matrix(~ ., X_p) # creates a design (or model) matrix
  head(X_p)
  # Here the design matrix has a column of ones for the intercept
  
  # here the 3rd and 5th beta are non-significant
  true_betas_p <- c(1, 0.3, 0.11, 0.5, 0.6, -0.2) # includes intercept
  true_gamma_p <- c(1,1,0,1,1,0) # inclusion parameter, gammma[1] is always 1 as this is the intercept.
  true_gammabeta_p <- true_gamma_p*true_betas_p
  
  p <-  array(ilogit(X_p %*% true_gammabeta_p), dim= c(M,J,T)) # occupancy probability
  mean(apply(p, 2, mean))
...

{
  ...
  for (i in 1:ncov) {
    if(classCovariates[i]){ # if it's a numerical covariates
      ...
      num_levels <- length(unique(originalX_p[,i]))
      C[l + 1:(num_levels-1), l + 1:(num_levels-1)] <- .5 * (diag(1, nrow = (num_levels-1)) +
                                                               matrix(1, nrow = (num_levels-1), ncol = (num_levels-1)))
      l <- l + num_levels - 1
    }
  ...
  numVars = ncov+1 #number of covariates (including the intercept)
...
win.data <- list(S= M*T, D=S, nsamples = length(y), nsites = M, numVars = numVars, b = b_psi, B = B_psi, zindex = zindex )
str(win.data)
...
nimOcc <- nimbleCode({
  ...
  # BVS on psi
  Omega_psi[1:nsites] ~ dPG()
  gamma_psi[1:numVars]  ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars])  
  beta_psi[1:numVars] ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars]) 
  gamma_beta_psi[1:numVars] <- gamma_psi[1:numVars]*beta_psi[1:numVars] 
  linpred_psi[1:nsites] <- (X_psi[1:nsites, 1:numVars] %*% gamma_beta_psi[1:numVars])[1:nsites,1]  
  
  
  # BVS on phi
  Omega_phi[1:D] ~ dPG()
  gamma_phi[1:numVars]  ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars])  
  beta_phi[1:numVars] ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars]) 
  gamma_beta_phi[1:numVars] <- gamma_phi[1:numVars]*beta_phi[1:numVars] 
  linpred_phi[1:D] <- (X_phi[1:D, 1:numVars] %*%  gamma_beta_phi[1:numVars])[1:D,1]  
  
  # BVS on colonization
  Omega_eta[1:D] ~ dPG()
  gamma_eta[1:numVars]  ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars])  
  beta_eta[1:numVars] ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars]) 
  gamma_beta_eta[1:numVars] <- gamma_eta[1:numVars]*beta_eta[1:numVars] 
  linpred_eta[1:D] <- (X_eta[1:D, 1:numVars] %*%  gamma_beta_eta[1:numVars])[1:D,1]  
  
  
  # BVS on p
  Omega_p[1:nsamples] ~ dPG()
  gamma_p[1:numVars]  ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars])  
  beta_p[1:numVars] ~ dmnorm(b[1:numVars], B[1:numVars, 1:numVars]) 
  gamma_beta_p[1:numVars] <- gamma_p[1:numVars]*beta_p[1:numVars] 
  linpred_p[1:nsamples] <- (X_p[1:nsamples, 1:numVars] %*% gamma_beta_p[1:numVars])[1:nsamples,1]  
...
Omega_p = pgdraw(rep(1, each = N),0.5*up)
up = matrix(1,S,1)
Omega_phi = pgdraw(rep(1, each = S),0.5*up)
up = matrix(1,S,1)
Omega_eta = pgdraw(rep(1, each = S),0.5*up)
u = matrix(1,M,1)
Omega_psi = pgdraw(rep(1, each = M),0.5*u)


ints = list(z = zints, eta =runif(1,0,1), psi = runif(1,0,1), beta_phi = rep(0, numVars), gamma_phi = rep(1,numVars), Omega_phi = Omega_phi ,beta_eta = rep(0, numVars), gamma_eta = rep(1,numVars), Omega_eta= Omega_eta ,beta_p = rep(0, numVars), gamma_p = rep(1,numVars), Omega_p= Omega_p, beta_psi = rep(0, numVars), gamma_psi = rep(1,numVars), Omega_psi= Omega_psi )
Occmodel = nimbleModel(nimOcc, constants = win.data,data = list(y=y, X_p=X_p, X_phi= X_phi, X_eta= X_eta, X_psi = X_psi), inits = ints)
Occmodel$calculate()
